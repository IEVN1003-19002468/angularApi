...
export default class EditarComponent {
  ...
  constructor(private fb: FormBuilder,private location: Location,public Utl: ProyectoapiService, private router:Router){}
  ...
  ngOnInit(){
    ...
    this.Utl.getAlumno(parseInt(this.tem[3])).subscribe(
      {
        next: response=>{
 ...
 modificar(){
  this.Utl.modificarAlumno(this.tem[3],this.regAlumno).subscribe({
    next:()=>console.log(),
